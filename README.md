# RealWise - A real state management system <!-- omit from toc -->

RealWise is a real estate management system based on Django and Next.js, combining both frameworks under one monorepository, including best practices.
...
To start using this project, it is needed to clone the repository to your local machine and then run `docker compose`, which will take care about the installation process. The only prerequisite for starting the project is to have `docker compose` installed and preconfiguring files with environment variables.

```bash
git clone https://github.com/pedropcamellon/real-wise.git
cd real-wise
```
...
Project structure on the front end, it is quite different from the directory hierarchy in the backend. The project counts with an option that front end have multiple front ends available on various domains or ports.
...
For the authentication, this project uses **django-simplejwt** and **next-auth** package to provide simple REST based JWT authentication. On the backend, there is no configuraton related to django-simplejwt so everything is set to default values.
...
The second option how to create new user account is to register it on the front end. The project provides a simple registration form. After account registration. Superuser needs to access Django admin and activate an account.
...
Currently Next.js server actions in folder `frontend/apps/web/src/actions/` are responsible for communication with the backend. When the server action is hit from the client, it fetches required data from Django API backend.

### API Client

The query between server action and Django backend is handled by using an API client generated by `openapi-typescript-codegen` package. There is a function `getApiClient` available in `frontend/apps/web/src/lib/api.ts` which already implements default options and authentication tokens.
...
By default, the project includes Swagger for API schema which is available here `http://localhost:8000/api/schema/swagger-ui/`. Swagger can be disabled by editing `urls.py` and removing `SpectacularSwaggerView`.

### Client side requests

At the moment, this project does not contain any examples of client side requests towards the backend. All the requests are handled by server actions. For client side requests, it is recommended to use [react-query](https://github.com/TanStack/query).
